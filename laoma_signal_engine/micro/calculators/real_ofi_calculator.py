...
from laoma_signal_engine.micro.runtime_logging import rate_limited_summary
...
class RealOFICalculator:
    ...
    def _reset_session(self):
        """重置会话状态，清空历史数据避免跨段污染"""
        rate_limited_summary(
            "ofi_session_reset",
            prefix="[OFI_SESSION_RESET_SUMMARY]",
            window_sec=60,
        )
        self.ofi_hist.clear()
        self.ema_ofi = None
    ...
    def update_with_snapshot(
        ...
        if hasattr(self, '_last_event_time_ms') and event_time_ms is not None:
            ...
            if time_gap > getattr(self, 'reset_on_gap_ms', 2000):
                rate_limited_summary(
                    "ofi_time_gap_reset",
                    prefix="[OFI_TIME_GAP_RESET_SUMMARY]",
                    window_sec=60,
                    max_value=float(time_gap),
                    extra={"threshold_ms": getattr(self, "reset_on_gap_ms", 2000)},
                )
                self._reset_session()
                session_reset = True
        ...
        if (hasattr(self, 'per_symbol_window') and self.per_symbol_window and 
            ...
            rate_limited_summary(
                "ofi_symbol_reset",
                prefix="[OFI_SYMBOL_RESET_SUMMARY]",
                window_sec=60,
                extra={"from": self.symbol, "to": current_symbol.upper()},
            )
            self._reset_session()
            session_reset = True
        ...
        if (hasattr(self, 'reset_on_session_change') and self.reset_on_session_change and 
            ...
            if hasattr(self, '_last_utc_day'):
                ...
                if current_utc_day != self._last_utc_day:
                    rate_limited_summary(
                        "ofi_day_reset",
                        prefix="[OFI_DAY_RESET_SUMMARY]",
                        window_sec=60,
                        extra={"from": self._last_utc_day, "to": current_utc_day},
                    )
                    self._reset_session()
                    session_reset = True
        ...
        if len(arr) < warmup_threshold:
            ...
            log.debug(
                "[OFI_WARMUP] samples=%s threshold=%s z_ofi=None",
                len(arr),
                warmup_threshold,
            )
        else:
            ...
            if s < self.std_floor:
                ...
                print(f"[OFI_STD_FLOOR] std below floor, using floor value: {s:.6f}", flush=True)
            ...
            if hasattr(self, 'ofi_hist') and len(self.ofi_hist) > 10:
                ...
                if mad > 0 and abs(ofi_val - median_ofi) > mad_threshold:
                    ...
                    if self.debug_winsorize:
                        log.debug(
                            "[OFI_WINSORIZE] winsorized: %.3f -> %.3f",
                            ofi_val_original,
                            ofi_val,
                        )
            ...
            if z_clip is not None and z_clip > 0 and z_clip < 1e6:
                if abs(z_ofi) > z_clip:
                    ...
                    print(f"[OFI_Z_CLIP] clipped z-score: {pre:.3f} -> {z_ofi:.3f}", flush=True)
                # 重置Z-score为零的计数
