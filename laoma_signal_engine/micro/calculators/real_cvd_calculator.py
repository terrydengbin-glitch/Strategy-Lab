...
from laoma_signal_engine.micro.runtime_logging import log_once
...
class RealCVDCalculator:
    ...
    def _apply_symbol_specific_config(self) -> None:
        ...
        if symbol_upper == "BTCUSDT":
            ...
            log_once(
                "cvd_symbol_config_BTCUSDT",
                "[CVD] Applied BTC-specific config: winsor_limit=1.9, mad_multiplier=1.75",
            )
            ...
            log_once(
                "cvd_symbol_config_ETHUSDT",
                "[CVD] Applied ETH-specific config: winsor_limit=2.3, mad_multiplier=1.70",
            )
            
        else:
            # 其余品种使用全局默认
            log_once(
                f"cvd_symbol_config_{symbol_upper}",
                f"[CVD] Using global default config for {symbol_upper}: winsor_limit=2.0, mad_multiplier=1.70",
            )
    ...
    def set_regime_mode(self, mode: str, *, override_thresholds: bool = False) -> None:
        ...
        if mode == "quiet":
            ...
            if override_thresholds:
                ...
                self.cfg.mad_multiplier = 1.70
                log_once(
                    f"cvd_regime_{self.symbol}_quiet_override",
                    "[CVD] Switched to QUIET regime: hybrid scale, winsor=2.0, mad=1.70 (overriding symbol config)",
                )
            else:
                log_once(
                    f"cvd_regime_{self.symbol}_quiet_keep",
                    f"[CVD] Switched to QUIET regime: hybrid scale (keeping symbol-specific winsor={self.cfg.winsor_limit}, mad={self.cfg.mad_multiplier})",
                )
            ...
            if override_thresholds:
                ...
                self.cfg.mad_multiplier = 1.70
                log_once(
                    f"cvd_regime_{self.symbol}_active_override",
                    "[CVD] Switched to ACTIVE regime: ewma scale, winsor=2.7, mad=1.70 (overriding symbol config)",
                )
            else:
                log_once(
                    f"cvd_regime_{self.symbol}_active_keep",
                    f"[CVD] Switched to ACTIVE regime: ewma scale (keeping symbol-specific winsor={self.cfg.winsor_limit}, mad={self.cfg.mad_multiplier})",
                )
    ...
    def _print_effective_config(self) -> None:
        ...
        log_once(f"cvd_effective_config_header_{self.symbol}", f"[CVD] Effective config for {self.symbol}:")
        log_once(f"cvd_effective_config_z_mode_{self.symbol}", f"  Z_MODE={self.cfg.z_mode}")  # 防止误配置
        log_once(f"cvd_effective_config_half_life_{self.symbol}", f"  HALF_LIFE_TRADES={self.cfg.half_life_trades}")
        log_once(f"cvd_effective_config_winsor_{self.symbol}", f"  WINSOR_LIMIT={self.cfg.winsor_limit}")
        log_once(f"cvd_effective_config_stale_{self.symbol}", f"  STALE_THRESHOLD_MS={self.cfg.stale_threshold_ms}")
        log_once(f"cvd_effective_config_freeze_{self.symbol}", f"  FREEZE_MIN={self.cfg.freeze_min}")
        log_once(f"cvd_effective_config_soft_freeze_{self.symbol}", f"  SOFT_FREEZE_MS={self.cfg.soft_freeze_ms}")  # 软冻结阈值
        log_once(f"cvd_effective_config_hard_freeze_{self.symbol}", f"  HARD_FREEZE_MS={self.cfg.hard_freeze_ms}")  # 硬冻结阈值
        log_once(f"cvd_effective_config_scale_mode_{self.symbol}", f"  SCALE_MODE={self.cfg.scale_mode}")
        log_once(f"cvd_effective_config_ewma_fast_{self.symbol}", f"  EWMA_FAST_HL={self.cfg.ewma_fast_hl}")
        # 打印归一化后的权重
        ...
        if w_sum > 1e-9:
            ...
            log_once(f"cvd_effective_config_scale_fast_{self.symbol}", f"  SCALE_FAST_WEIGHT={self.cfg.scale_fast_weight} -> {w_fast_norm:.3f} (normalized)")
            log_once(f"cvd_effective_config_scale_slow_{self.symbol}", f"  SCALE_SLOW_WEIGHT={self.cfg.scale_slow_weight} -> {w_slow_norm:.3f} (normalized)")
        else:
            log_once(f"cvd_effective_config_scale_fast_raw_{self.symbol}", f"  SCALE_FAST_WEIGHT={self.cfg.scale_fast_weight} (slow={self.cfg.scale_slow_weight})")
        log_once(f"cvd_effective_config_mad_window_{self.symbol}", f"  MAD_WINDOW_TRADES={self.cfg.mad_window_trades}")
        log_once(f"cvd_effective_config_mad_scale_{self.symbol}", f"  MAD_SCALE_FACTOR={self.cfg.mad_scale_factor}")
        log_once(f"cvd_effective_config_mad_multiplier_{self.symbol}", f"  MAD_MULTIPLIER={self.cfg.mad_multiplier}")
