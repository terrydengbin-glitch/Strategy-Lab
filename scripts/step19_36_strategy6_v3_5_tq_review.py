...
STEP21_RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_5_walk_forward_STEP21_57_result.json"
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_5_tq_STEP19_36_result.json"
...
def write_report(payload: dict[str, Any]) -> Path:
    ...
    path = REPORT_DIR / f"STEP19.36_strategy6_v3_5_tq_review_{ts}.md"
    lines = [
        "# STEP19.36 Strategy6 V3.5 Trade Quality Review",
        "",
        f"- generated_at: `{_now()}`",
    ...
    for package in payload.get("packages") or []:
        ...
        lines.extend(
            [
                ...
                f"- tq_avg_win_R: `{stats.get('avg_win_R')}`",
...
def main() -> None:
    ...
    if not experiment_id:
        raise SystemExit("STEP21.57 result has no experiment_id")
    ...
    for param in _selected_parameter_sets(step21, args.top_n):
        mat = materialize_payload(ROOT, experiment_id=experiment_id, strategy_line="strategy6", parameter_set_id=param, top_n=1, limit=args.limit, dry_run=False, force=args.force)
        pkg_key = (mat.get("package_keys") or [None])[0]
        summary = summary_payload(ROOT, experiment_id=experiment_id, strategy_line="strategy6", parameter_set_id=param, package_key=pkg_key, limit=args.limit)
        packages.append({"parameter_set_id": param, "package_key": pkg_key, "materialize": mat, "summary": summary, "walk_forward": wf_by_param.get(param, {})})
    payload = {"schema_version": "step19.36-strategy6-v3-5-tq-review-v1", "experiment_id": experiment_id, "source_result": str(args.result), "packages": packages, "generated_at": _now()}
