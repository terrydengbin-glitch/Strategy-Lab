...
GRID_PATH = ROOT / "DATA" / "backtest" / "strategy6_v2_focused_30params_STEP21_51.json"
PROGRESS_PATH = ROOT / "DATA" / "backtest" / "strategy6_v2_focused_STEP21_51_progress.json"
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v2_focused_STEP21_51_result.json"
...
def _stable_id(payload: dict[str, Any]) -> str:
    ...
    return "s6v2_" + hashlib.sha1(raw).hexdigest()[:16]


def build_grid() -> list[dict[str, Any]]:
    candidates: list[dict[str, Any]] = []
    direction_mins = [52, 58, 64]
    uncertain_scores = [44, 48]
    chase_caps = [45.0, 65.0, 85.0]
    distance_caps = [70.0, 95.0]
    rr_caps = [0.45, 0.6, 0.8]
    max_waits = [8, 12]
    exit_profiles = [
        {
            "medium_quality_loss_cap_R": 0.90,
            "medium_quality_first_tp_R": 0.55,
            "medium_quality_protect_after_mfe_R": 0.45,
            "medium_quality_trail_after_mfe_R": 0.35,
        },
        {
            "medium_quality_loss_cap_R": 0.95,
            "medium_quality_first_tp_R": 0.65,
            "medium_quality_protect_after_mfe_R": 0.55,
            "medium_quality_trail_after_mfe_R": 0.45,
        },
    ]
    for direction_min in direction_mins:
        for uncertain in uncertain_scores:
            for chase in chase_caps:
                for distance in distance_caps:
                    for rr_cap in rr_caps:
                        for max_wait in max_waits:
                            for exit_profile in exit_profiles:
                                strategy6 = {
                                    "strategy6_version": "v2",
                                    "v2_min_direction_acceptance_score": direction_min,
                                    "v2_uncertain_direction_score": uncertain,
                                    "v2_hard_deny_direction_score": max(30, uncertain - 12),
                                    "v2_max_chase_bps": chase,
                                    "v2_adverse_1m_deny_bps": 18.0,
                                    "v2_reversal_1m_wait_bps": 8.0,
                                    "v2_distance_from_mean_max_bps": distance,
                                    "v2_high_quality_score": 74,
                                    "v2_medium_quality_score": direction_min,
                                    "strategy6_wait_rebound_enabled": True,
                                    "strategy6_wait_max_minutes": max_wait,
                                    "strategy6_wait_pullback_min_bps": 8.0,
                                    "strategy6_wait_pullback_max_bps": 55.0,
                                    ...
                                    **exit_profile,
                                    ...
                                    "low_quality_first_tp_R": 0.45,
                                    "low_quality_protect_after_mfe_R": 0.40,
                                    "low_quality_trail_after_mfe_R": 0.30,
                                    "high_quality_loss_cap_R": 1.05,
                                    "high_quality_protect_after_mfe_R": 0.85,
                                    "high_quality_trail_after_mfe_R": 0.65,
                                }
                                params = {
                                    "strategy_line": "strategy6",
                                    "min_score": max(48.0, float(direction_min) - 6.0),
                                    "target_rr": rr_cap,
    ...
    # Keep a deliberately small, diverse grid. Ordering alternates strict/loose enough
    # that smoke mode still exercises the V2 contract.
    profile_indexes = [0, 1, 2, 5, 8, 13, 21, 34, 55, 89]
    ...
    for idx in profile_indexes:
        ...
        if item["parameter_set_id"] not in {p["parameter_set_id"] for p in picks}:
            picks.append(item)
    for item in candidates[0:: max(1, len(candidates) // 40)]:
        if item["parameter_set_id"] not in {p["parameter_set_id"] for p in picks}:
            picks.append(item)
        if len(picks) >= 30:
            break
...
def ensure_grid() -> list[dict[str, Any]]:
    ...
    GRID_PATH.write_text(
        json.dumps(
            {
                "schema_version": "step21.51-strategy6-v2-focused-grid-v1",
                "purpose": "focused Strategy6 V2 matrix; V1/default strategy behavior remains unchanged",
                "parameter_sets": grid,
                "generated_at": _now(),
            },
            ensure_ascii=False,
...
def write_report(result: dict[str, Any], *, smoke: bool) -> Path:
    ...
    path = REPORT_DIR / f"STEP21.51_strategy6_v2_focused_matrix_{ts}.md"
    best = result.get("best") or {}
    metrics = best.get("metrics") or {}
    leaderboard = result.get("leaderboard") or []
    lines = [
        "# STEP21.51 Strategy6 V2 Focused Matrix Backtest Report",
        "",
        f"- generated_at: `{_now()}`",
    ...
    lines.extend(
        [
            ...
            "## Boundary",
            "",
            "- Only Strategy6 V2 offline backtest was executed.",
            "- No live config, paper ledger, Feishu, or other strategy line was changed.",
            "- This report is evidence for STEP19.29 Trade Quality review, not a production recommendation.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def main() -> None:
    ...
    parser.add_argument("--smoke", action="store_true", help="Run 5 symbols * 3 params")
    ...
    parser.add_argument("--skip-download", action="store_true", help="Do not fill missing 1m kline cache before running")
    ...
    if args.regenerate_grid and GRID_PATH.exists():
        GRID_PATH.unlink()
    grid = ensure_grid()
    max_symbols = args.max_symbols if args.max_symbols is not None else (5 if args.smoke else 100)
    max_sets = args.max_sets if args.max_sets is not None else (3 if args.smoke else 30)
    ...
    if not args.skip_download:
        ...
        if missing:
            print(
                json.dumps(
                    {"phase": "download_missing_klines", "missing_count": len(missing), "symbols": missing[:10]},
                    ensure_ascii=False,
                ),
                flush=True,
            )
                ...
                json.dumps(
                    {
                        "done_count": done,
                        "total_count": total,
                        "current_parameter_set_id": progress.get("current_parameter_set_id"),
                        "current_symbol_shard": progress.get("current_symbol_shard"),
                        "persisted_order_count": progress.get("persisted_order_count"),
                        "max_workers": progress.get("max_workers"),
    ...
    result = {
        "schema_version": "step21.51-result-v1",
