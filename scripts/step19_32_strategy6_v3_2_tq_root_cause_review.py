...
STEP21_RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_2_focused_STEP21_54_result.json"
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_2_tq_STEP19_32_result.json"
...
def _load_result(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise SystemExit(f"missing STEP21.54 result: {path}")
...
def write_report(payload: dict[str, Any]) -> Path:
    ...
    path = REPORT_DIR / f"STEP19.32_strategy6_v3_2_tq_root_cause_{ts}.md"
    lines = [
        "# STEP19.32 Strategy6 V3.2 Trade Quality Root Cause Review",
        "",
        f"- generated_at: `{_now()}`",
    ...
    lines.extend(
        [
            "## Boundary",
            "",
            "- This review only materializes bounded Strategy6 V3.2 backtest samples into Backtest Trade Quality tables.",
...
def main() -> None:
    ...
    if not experiment_id:
        raise SystemExit("STEP21.54 result has no experiment_id")
    ...
    payload = {
        "schema_version": "step19.32-strategy6-v3-2-tq-review-v1",
        "experiment_id": experiment_id,
