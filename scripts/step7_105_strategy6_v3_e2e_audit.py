...
STEP21_RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_focused_STEP21_52_result.json"
...
def write_report(payload: dict[str, Any]) -> Path:
    ...
    path = REPORT_DIR / f"STEP7.105_strategy6_v3_e2e_audit_{ts}.md"
    ...
    lines = [
        "# STEP7.105 Strategy6 V3 E2E Audit Report",
        "",
    ...
    lines.extend(
        [
            "## Contract Audit",
            "",
            "- Strategy6 V3 is opt-in via `strategy6_version=v3`; V1/V2 remain available.",
            "- V3 evidence fields are carried through evaluator guards, backtest shadow orders, fill features, and TQ samples.",
            "- No live config, paper ledger, Feishu, or Strategy1/2/4/5 runtime behavior was changed.",
            "- Smoke evidence proves the V3 chain runs; focused/full matrix promotion still requires separate quality acceptance.",
            "",
            "## Verification Tails",
            "",
            "### pytest",
...
def main() -> None:
    compileall = _run(
        [
            ...
            "laoma_signal_engine/backtest/p21_trade_quality.py",
            "scripts/step21_52_strategy6_v3_focused_matrix_run.py",
            "scripts/step19_30_strategy6_v3_tq_root_cause_review.py",
        ]
    )
    pytest = _run(
        [
    ...
    payload = {
        "schema_version": "step7.105-strategy6-v3-e2e-audit-v1",
        "generated_at": _now(),
