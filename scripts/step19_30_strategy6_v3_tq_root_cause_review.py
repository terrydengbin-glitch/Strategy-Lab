...
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_tq_STEP19_30_result.json"
...
def _load_step21_result(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise SystemExit(f"missing STEP21.52 result: {path}")
...
def write_report(payload: dict[str, Any]) -> Path:
    ...
    path = REPORT_DIR / f"STEP19.30_strategy6_v3_tq_root_cause_{ts}.md"
    lines = [
        "# STEP19.30 Strategy6 V3 Trade Quality Root Cause Review",
        "",
        f"- generated_at: `{_now()}`",
    ...
    for package in payload.get("packages") or []:
        ...
        lines.extend(["", "### Entry / Context Labels", ""])
        for block_name in ("entry_quality_attribution", "entry_context_v3_attribution"):
    ...
    lines.extend(
        [
            "## Boundary",
            "",
            "- This task only materializes bounded backtest samples into Backtest Trade Quality tables.",
            "- No live config, paper order, or runtime strategy behavior is changed.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def main() -> None:
    ...
    if not experiment_id:
        raise SystemExit("STEP21.52 result has no experiment_id")
    ...
    payload = {
        "schema_version": "step19.30-strategy6-v3-tq-review-v1",
        "experiment_id": experiment_id,
