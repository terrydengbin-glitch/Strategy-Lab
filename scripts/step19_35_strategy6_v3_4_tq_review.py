...
STEP21_RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_4_walk_forward_STEP21_56_result.json"
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_4_tq_STEP19_35_result.json"
...
def write_report(payload: dict[str, Any]) -> Path:
    ...
    path = REPORT_DIR / f"STEP19.35_strategy6_v3_4_tq_review_{ts}.md"
    lines = [
        "# STEP19.35 Strategy6 V3.4 Trade Quality Review",
        "",
        f"- generated_at: `{_now()}`",
    ...
    for package in payload.get("packages") or []:
        ...
        lines.extend(
            [
                "",
                "| hour_bucket | count | win_rate | avg_R | total_R |",
                "| --- | ---: | ---: | ---: | ---: |",
            ]
        )
        for item in _items(summary, "hour_attribution")[:10]:
            lines.append(
                f"| `{_first(item, 'hour', 'key', 'label', 'name')}` | "
                f"{_first(item, 'count', 'sample_count', 'trade_count')} | {item.get('win_rate')} | "
                f"{_first(item, 'avg_R', 'avg_net_R')} | {item.get('total_R')} |"
            )
        lines.append("")
    lines.extend(
        [
...
def main() -> None:
    ...
    if not experiment_id:
        raise SystemExit("STEP21.56 result has no experiment_id")
    ...
    payload = {
        "schema_version": "step19.35-strategy6-v3-4-tq-review-v1",
        "experiment_id": experiment_id,
