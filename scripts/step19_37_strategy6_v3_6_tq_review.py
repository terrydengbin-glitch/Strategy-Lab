...
STEP21_RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_6_walk_forward_STEP21_58_result.json"
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_6_tq_STEP19_37_result.json"
...
def write_report(payload: dict[str, Any]) -> Path:
    ...
    path = REPORT_DIR / f"STEP19.37_strategy6_v3_6_tq_review_{ts}.md"
    lines = [
        "# STEP19.37 Strategy6 V3.6 Trade Quality Review",
        "",
        f"- generated_at: `{_now()}`",
...
def main() -> None:
    ...
    if not experiment_id:
        raise SystemExit("STEP21.58 result has no experiment_id")
    ...
    payload = {"schema_version": "step19.37-strategy6-v3-6-tq-review-v1", "experiment_id": experiment_id, "source_result": str(args.result), "packages": packages, "generated_at": _now()}
