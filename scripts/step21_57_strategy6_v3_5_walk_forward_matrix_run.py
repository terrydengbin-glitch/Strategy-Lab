...
GRID_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_5_walk_forward_STEP21_57_grid.json"
PROGRESS_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_5_walk_forward_STEP21_57_progress.json"
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_5_walk_forward_STEP21_57_result.json"
...
def _stable_id(payload: dict[str, Any]) -> str:
    return "s6v35_" + hashlib.sha1(json.dumps(payload, ensure_ascii=False, sort_keys=True).encode("utf-8")).hexdigest()[:16]


def _params(*, first_tp: float, loss_cap: float, scratch_min: int, min_push: float, early_adverse: float, lock_trigger: float, lock_floor: float) -> dict[str, Any]:
    strategy6 = {
        ...
        "first_tp_R": first_tp,
        "v3_5_scratch_after_minutes": scratch_min,
        "v3_5_min_push_R": min_push,
        "v3_5_early_adverse_R": early_adverse,
        "v3_5_lock_trigger_R": lock_trigger,
        "v3_5_lock_floor_R": lock_floor,
    }
    return {
...
def build_grid() -> list[dict[str, Any]]:
    ...
    for lock_trigger, lock_floor in ((0.40, 0.05), (0.50, 0.10), (0.60, 0.15)):
        for scratch_min, min_push, early_adverse in ((2, 0.10, 0.40), (3, 0.12, 0.45)):
            for first_tp, loss_cap in ((0.60, 0.75), (0.70, 0.85), (0.80, 0.90)):
                raw.append(
                    _params(
                        first_tp=first_tp,
                        loss_cap=loss_cap,
...
def ensure_grid(regenerate: bool = False) -> list[dict[str, Any]]:
    ...
    grid = build_grid()
    GRID_PATH.write_text(json.dumps({"schema_version": "step21.57-strategy6-v3-5-grid-v1", "parameter_sets": grid, "generated_at": _now()}, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    return grid
...
def write_report(result: dict[str, Any], *, smoke: bool) -> Path:
    ...
    path = REPORT_DIR / f"STEP21.57_strategy6_v3_5_walk_forward_matrix_{ts}.md"
    selected = (result.get("walk_forward") or {}).get("selected") or {}
    lines = [
        "# STEP21.57 Strategy6 V3.5 Walk-Forward Matrix Report",
        "",
        f"- generated_at: `{_now()}`",
    ...
    lines.extend(["", "## Boundary", "", "- Test split is holdout only and is not used for parameter selection.", "- V3.5 is opt-in and does not mutate live strategy config."])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def main() -> None:
    ...
    result = {"schema_version": "step21.57-strategy6-v3-5-walk-forward-result-v1", "mode": "smoke" if args.smoke else "focused", "no_lookahead_contract": {"entry_feature_contract_field": "strategy6_v3_5_known_at_contract", "test_split_used_for_selection": False}, "grid_path": str(GRID_PATH), "progress_path": str(PROGRESS_PATH), "walk_forward": wf, **payload}
