...
GRID_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_6_walk_forward_STEP21_58_grid.json"
PROGRESS_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_6_walk_forward_STEP21_58_progress.json"
RESULT_PATH = ROOT / "DATA" / "backtest" / "strategy6_v3_6_walk_forward_STEP21_58_result.json"
...
def _stable_id(payload: dict[str, Any]) -> str:
    return "s6v36_" + hashlib.sha1(json.dumps(payload, ensure_ascii=False, sort_keys=True).encode("utf-8")).hexdigest()[:16]
...
def _params(
    *,
    ...
    strategy6 = {
        ...
        "v3_4_min_followthrough_5m_bps": 4.0,
        "v3_4_min_volume_z": 0.8,
        "v3_4_long_max_range_pos": 0.88,
        "v3_4_short_min_range_pos": 0.12,
        "v3_4_max_distance_to_mean_bps": 72.0,
        "v3_4_no_edge_action": "wait",
        "v3_4_range_noise_action": "wait_rebound",
        "v3_5_no_edge_aligned_5m_bps": 4.0,
        "v3_5_no_edge_volume_z": 0.7,
        "v3_5_hard_wrong_adverse_1m_bps": 8.0,
        "v3_5_hard_wrong_adverse_3m_bps": 18.0,
        "v3_5_rebound_range_long": 0.82,
        "v3_5_rebound_range_short": 0.18,
        "v3_5_profit_lock_min_aligned_5m_bps": 8.0,
        "v3_6_hard_wrong_1m_bps": hard_1m,
        "v3_6_hard_wrong_3m_bps": hard_3m,
        "v3_6_min_followthrough_5m_bps": follow_5m,
        "v3_6_min_volume_z": min_vol,
        "v3_6_no_edge_action": no_edge_action,
        "v3_6_base_v3_4_followthrough_bps": -999.0,
        "v3_6_base_v3_4_min_volume_z": 0.0,
        ...
        "first_tp_R": first_tp,
        "v3_5_scratch_after_minutes": 3,
        "v3_5_min_push_R": 0.12,
        "v3_5_early_adverse_R": 0.45,
        "v3_5_lock_trigger_R": 0.50,
        "v3_5_lock_floor_R": 0.10,
    }
    return {
...
def build_grid() -> list[dict[str, Any]]:
    ...
    for no_edge_action in ("wait", "deny"):
        for hard_1m, hard_3m in ((8.0, 18.0), (10.0, 22.0), (12.0, 26.0)):
            for follow_5m, min_vol in ((4.0, 0.8), (6.0, 0.9), (8.0, 1.0)):
                for first_tp, loss_cap in ((0.70, 0.85), (0.80, 0.90)):
                    raw.append(
                        _params(
                            first_tp=first_tp,
                            loss_cap=loss_cap,
                            ...
                            follow_5m=follow_5m,
                            min_vol=min_vol,
                            no_edge_action=no_edge_action,
                        )
                    )
    return [{"parameter_set_id": _stable_id(params), "parameters": params} for params in raw]
...
def ensure_grid(regenerate: bool = False) -> list[dict[str, Any]]:
    ...
    grid = build_grid()
    GRID_PATH.write_text(json.dumps({"schema_version": "step21.58-strategy6-v3-6-grid-v1", "parameter_sets": grid, "generated_at": _now()}, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    return grid
...
def write_report(result: dict[str, Any], *, smoke: bool) -> Path:
    ...
    path = REPORT_DIR / f"STEP21.58_strategy6_v3_6_walk_forward_matrix_{ts}.md"
    selected = (result.get("walk_forward") or {}).get("selected") or {}
    lines = [
        "# STEP21.58 Strategy6 V3.6 Walk-Forward Matrix Report",
        "",
        f"- generated_at: `{_now()}`",
    ...
    lines.extend(["", "## Boundary", "", "- Test split is holdout only and is not used for parameter selection.", "- V3.6 is opt-in and does not mutate live strategy config."])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def main() -> None:
    ...
    max_sets = args.max_sets if args.max_sets is not None else (12 if args.smoke else len(grid))
    ...
    result = {"schema_version": "step21.58-strategy6-v3-6-walk-forward-result-v1", "mode": "smoke" if args.smoke else "focused", "no_lookahead_contract": {"entry_feature_contract_field": "strategy6_v3_6_known_at_contract", "test_split_used_for_selection": False}, "grid_path": str(GRID_PATH), "progress_path": str(PROGRESS_PATH), "walk_forward": wf, **payload}
